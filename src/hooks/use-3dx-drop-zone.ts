...
export function extract3dxObject(dropArgs: string): DropItem[] {
	...
	if (dnd.protocol !== PROTOCOL) {
		logger.warn(
			`[DropZone] Unexpected protocol: "${dnd.protocol}" (expected "${PROTOCOL}")`,
		);
	}

	const data = dnd.data;
	const rawItems: DropItem[] =
...
export function use3dxDropZone<T extends HTMLElement>(
	...
	const register = useCallback(() => {
		...
		try {
			...
			logger.warn(
				"[use3dxDropZone] DataDragAndDrop not available (running outside 3DX platform)",
			);
			return;
		}
		...
		if (!dnd) {
			logger.warn(
				"[use3dxDropZone] DataDragAndDrop is not available on this platform",
			);
			return;
		}
