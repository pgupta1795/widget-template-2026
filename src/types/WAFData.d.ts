...
export type WAFDataResponseType = 'json' | 'text' | 'arraybuffer' | 'blob' | 'document';
...
export interface WAFDataBaseRequestOptions<TResponse = unknown, TError = Error> {
  method?: string;
  onComplete?(
    response: TResponse,
...
export interface WAFDataAuthenticatedRequestOptions<TResponse = unknown, TError = Error>
  extends WAFDataBaseRequestOptions<TResponse, TError> {}

export interface WAFDataProxifiedRequestOptions<TResponse = unknown, TError = Error>
  extends WAFDataBaseRequestOptions<TResponse, TError> {
