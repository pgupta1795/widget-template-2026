import type {
	...
	ParsedCollection,
	ParsedEndpoint,
} from "@/features/api-explorer/openapi/types";
...
export function useApiExplorer() {
	...
	if (!ctx)
		throw new Error("useApiExplorer must be used within ApiExplorerProvider");
	return ctx;
}

export function ApiExplorerProvider({ children }: { children: ReactNode }) {
	const { data: builtInCollections = [], isLoading: builtInLoading } =
		useBuiltInSpecs();
	...
	const [customCollections, setCustomCollections] = useState<
		ParsedCollection[]
	>([]);
	const [activeEndpoint, setActiveEndpoint] = useState<ParsedEndpoint | null>(
		null,
	);
	const [activeCollection, setActiveCollection] =
		useState<ParsedCollection | null>(null);
	...
	const loadEndpoint = useCallback(
		(collection: ParsedCollection, endpoint: ParsedEndpoint) => {
			...
			const scParam = endpoint.queryParams.find(
				(p) => p.name === "SecurityContext",
			);
			...
			if (scParam !== undefined) {
				qParams.push(
					newKv(
						...
						sc,
						false,
						"Platform security context (editable)",
					),
				);
			}
			...
			if (!hParams.some((h) => h.key === "ENO_CSRF_TOKEN")) {
				hParams.unshift({
					...newKv("ENO_CSRF_TOKEN", "", true, "Auto-managed by WAF pipeline"),
					readOnly: true,
				});
			}
	...
	const sendRequest = useCallback(() => {
		...
		for (const p of nextQParams) {
			if (p.enabled && p.key && p.key !== "ENO_CSRF_TOKEN")
	...
	const onObjectDrop = useCallback((result: DropItem[]) => {
		...
		toast.success(
			`Object with name ${item.displayName} having type ${item.objectType} and id ${id} is dropped successfully`,
		);
			...
			try {
				...
				if (
					...
					return JSON.stringify(
						fillIdFields(parsed as Record<string, unknown>, id),
						null,
						2,
					);
				}
