	...
	for (const [mime, mediaType] of Object.entries(resolved.content ?? {})) {
		content[mime] = {
			schema: mediaType.schema
				? resolveSchema(mediaType.schema, spec)
				: undefined,
		};
	}
...
export function parseSpec(raw: unknown, filename: string): ParsedCollection {
	...
	if (!spec.paths) {
		throw new OpenApiParseError(
			filename,
			'Missing "paths" — not an OpenAPI spec',
		);
	}
	...
	for (const [path, pathItem] of Object.entries(spec.paths ?? {})) {
		...
		for (const method of VALID_METHODS) {
			const op = (pathItem as any)[method.toLowerCase()] as
	...
	const endpointCount = parsedTags.reduce(
		(sum, t) => sum + t.endpoints.length,
		0,
	);
