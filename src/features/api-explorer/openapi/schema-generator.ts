...
export function generateExample(schema: Schema, depth = 0): unknown {
	...
	if (s.allOf) {
		return s.allOf.reduce<Record<string, unknown>>(
			(acc, sub) => ({
				...acc,
				...(generateExample(sub, depth + 1) as object),
			}),
			{},
		);
	}
...
export function buildSchemaTree(
	...
	const type =
		schema.type ??
		(schema.properties ? "object" : s.items ? "array" : "unknown");
	...
	if (schema.properties) {
		...
		node.children = Object.entries(schema.properties).map(([key, prop]) => {
			if ("$ref" in prop)
				return { name: key, type: "$ref", description: "", required: false };
		...
		node.children = [
			buildSchemaTree(
				s.items as OpenAPIV3.SchemaObject,
				"[item]",
				false,
				depth + 1,
			),
