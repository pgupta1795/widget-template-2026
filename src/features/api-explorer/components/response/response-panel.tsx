...
import {Braces,Check,CheckCircle2,Copy,Database,List,Timer,XCircle} from 'lucide-react';
...
function HeadersTable({ headers }: { headers: Record<string, string> }) {
  ...
  if (entries.length === 0) {
    return <p className="text-xs text-muted-foreground text-center py-8">No headers returned</p>;
  }
    ...
    <div className="divide-y divide-border/50">
      {entries.map(([key, value]) => (
        ...
        >
          ...
          >
            {copiedKey === key ? <Check size={11} className="text-emerald-500" /> : <Copy size={11} />}
          </Button>
        </div>
      ))}
    </div>
  ...
  if (!response) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-muted-foreground/30">
        ...
        <p className="text-sm font-medium text-muted-foreground/50">No response yet</p>
        <p className="text-xs mt-1 text-muted-foreground/40">Send a request to see results</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 px-4 py-2 border-b border-border shrink-0 bg-card/30">
        ...
        <div className="flex items-center gap-1 text-xs text-muted-foreground">
          <span className="tabular-nums font-mono">{formatBytes(response.size)}</span>
        </div>
      ...
      {/* Tabs */}
      <Tabs defaultValue="body" className="flex-1 flex flex-col gap-0 overflow-hidden">
        <TabsList
          variant="line"
        ...
        <TabsContent value="body" className="flex-1 overflow-hidden mt-0 flex flex-col">
          ...
          <div className="flex items-center gap-1 px-3 py-1.5 border-b border-border/50 shrink-0">
            <span className="text-[10px] text-muted-foreground/60 mr-1">View:</span>
        ...
        <TabsContent value="headers" className="flex-1 overflow-hidden mt-0">
          <ScrollArea className="h-full">
            ...
            <div className="grid grid-cols-[auto_1fr_28px] gap-3 px-4 py-1.5 border-b border-border/50 bg-muted/20">
              <span className="text-[10px] uppercase tracking-wider font-semibold text-muted-foreground/60 min-w-35">Header</span>
              <span className="text-[10px] uppercase tracking-wider font-semibold text-muted-foreground/60">Value</span>
              <span />
            </div>
