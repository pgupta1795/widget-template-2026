			...
			{items.map((item) => (
				...
				>
					{item.readOnly ? (
						...
						<Checkbox
							...
							onCheckedChange={(checked) =>
								update(item.id, "enabled", checked === true)
							}
					...
					<Input
						...
						onChange={(e) =>
							!item.readOnly && update(item.id, "key", e.target.value)
						}
					...
					<Input
						...
						onChange={(e) =>
							!item.readOnly && update(item.id, "value", e.target.value)
						}
						className={`h-8 font-mono text-xs ${
							item.readOnly
								? "bg-muted/30 cursor-not-allowed text-muted-foreground italic"
								: "bg-card"
						}`}
					/>
					{item.readOnly ? (
