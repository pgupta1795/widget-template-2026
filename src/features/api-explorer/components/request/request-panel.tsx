		...
		<div className="flex items-center gap-0 font-mono text-xs flex-wrap min-w-0">
			{segments.map((seg, i) => {
				...
				return (
					<Fragment key={i}>
						{i > 0 && (
							<span className="text-muted-foreground/40 px-px select-none">
								/
							</span>
						)}
...
function MethodSelector() {
	const { activeEndpoint, overrideMethod, setOverrideMethod } =
		useApiExplorer();
	...
	return (
		<DropdownMenu>
			<DropdownMenuTrigger
				render={
					<button className="flex items-center gap-1 hover:opacity-80 transition-opacity cursor-pointer shrink-0 outline-none" />
				}
			>
			...
			<DropdownMenuContent side="bottom" align="start" className="min-w-28">
				{VALID_METHODS.map((m) => (
					<DropdownMenuItem
						key={m}
						onClick={() =>
							setOverrideMethod(m === activeEndpoint?.method ? null : m)
						}
				...
				<div className="border-b border-border shrink-0">
					...
					>
						...
						<span className="flex-1 text-left truncate">
							{activeEndpoint.summary || "Description"}
						</span>
			...
			>
				...
				>
					...
					>
						...
						{queryParamCount > 0 && (
							<Badge variant="info" className="text-[9px] px-1 h-3.5 min-w-0">
								{queryParamCount}
							</Badge>
						)}
					</TabsTrigger>
