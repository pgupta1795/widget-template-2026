      ...
      {activeCollections.map(col => (
        <div key={col.id} className="rounded-lg border border-border/50 overflow-hidden">
          <button
            ...
            className="w-full flex items-center gap-2 px-3 py-2 bg-card hover:bg-accent/30 transition-colors text-sm group cursor-pointer"
          >
            {expandedCollections.has(col.id) ? <ChevronDown size={13} /> : <ChevronRight size={13} />}
            <span className="flex-1 text-left truncate text-sidebar-foreground font-semibold text-xs">
              {col.name}
            </span>
            <span className="text-[9px] text-muted-foreground/60 bg-muted/60 px-1.5 py-0.5 rounded shrink-0 font-medium">
              {col.serviceType}
            </span>
