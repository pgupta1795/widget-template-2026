...
function EndpointItem({
	...
	const displayPath =
		segments.length > 3 ? "…/" + segments.slice(-2).join("/") : ep.path;
		...
		<TooltipProvider delay={400}>
			<Tooltip>
				...
				<TooltipContent side="right" className="max-w-64 text-left">
					<p className="font-mono text-[11px] font-semibold break-all">
						{ep.path}
					</p>
					{ep.description && (
	...
	return (
		<div>
			...
			>
				...
				<span className="ml-auto text-[9px] text-muted-foreground/50 bg-muted/50 px-1 rounded shrink-0">
	...
	const filteredCollections = useMemo(() => {
		...
		return activeCollections
			.map((col) => {
				...
				const matchingTags = col.tags
					.map((tag) => {
						if (tag.name.toLowerCase().includes(lowerSearch)) return tag;
						const matchingEndpoints = tag.endpoints.filter(
							(ep) =>
						...
						if (matchingEndpoints.length > 0)
							return { ...tag, endpoints: matchingEndpoints };
						return null;
					})
		...
		return (
			<div className="flex flex-col items-center justify-center py-10 px-4 text-center">
				...
				<p className="text-[11px] text-muted-foreground/60 mt-1">
					Switch to Browse tab to enable APIs
				</p>
			</div>
		);
	}
		...
		<div className="flex flex-col gap-2 pb-2">
			...
			<div className="space-y-1 px-1">
				{filteredCollections.map((col) => {
					const isExpanded =
						expandedCollections.has(col.id) || search.trim().length > 0;
					return (
						<div
							key={col.id}
							className="rounded-lg border border-border/50 overflow-hidden min-w-0"
						>
							<button
