        ...
        {filtered.map(col => (
          <div
            ...
            className={`flex items-center gap-2 px-2 py-2 rounded-lg transition-colors ${
              isActive(col.id) ? 'bg-primary/5 border border-primary/15' : 'hover:bg-sidebar-accent/50 border border-transparent'
            }`}
          >
            ...
            <div className="flex-1 min-w-0">
              <p className={`text-xs font-medium truncate leading-tight ${isActive(col.id) ? 'text-primary' : 'text-sidebar-foreground'}`}>
                {col.name}
              </p>
