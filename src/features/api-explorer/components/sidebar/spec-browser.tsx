    ...
    <div className="flex flex-col gap-2 pb-2">
      ...
      <div className="space-y-px px-1">
        {filtered.map(col => (
          <div
            key={col.id}
            className={`flex items-center gap-2 px-2 py-2 rounded-lg transition-colors ${isActive(col.id)? 'bg-primary/5 border border-primary/15':'hover:bg-sidebar-accent/50 border border-transparent'
              }`}
          >
            <div className="w-7 h-7 rounded-md bg-muted/60 flex items-center justify-center shrink-0">
              <Package size={12} className={isActive(col.id)? 'text-primary':'text-muted-foreground/60'} />
            </div>
            <div className="flex-1 min-w-0">
              <p className={`text-xs font-medium wrap-break-word leading-tight ${isActive(col.id)? 'text-primary':'text-sidebar-foreground'}`}>
                {col.name}
              </p>
        ...
        {filtered.length===0&&(
          <p className="text-xs text-muted-foreground text-center py-6">No APIs match your search</p>
        )}
      </div>
    </div>
  );
