...
import {
	getToken,
	invalidate as invalidateCsrf,
} from "@/services/core/csrf-manager";
...
import {
	wafAuthenticatedRequest,
	wafProxifiedRequest,
} from "@/services/core/waf-transport";
import {
	ServiceError,
	type HttpMethod,
	type RequestOptions,
...
export async function executePipeline<T>(
	...
	const execute = async (): Promise<ServiceResponse<T>> => {
		...
		if (useProxy) {
			return wafProxifiedRequest<T>(builtUrl, {
				type: "json",
				...wafOpts,
				headers,
				proxyType,
		...
		return wafAuthenticatedRequest<T>(builtUrl, {
			type: "json",
			...wafOpts,
			headers,
			timeout,
