...
export async function executePipeline<T>(
  ...
  const execute = async (): Promise<ServiceResponse<T>> => {
    ...
    if (useProxy) {
      return wafProxifiedRequest<T>(builtUrl, { type: 'json', ...wafOpts, headers, proxyType, timeout, method });
    }
    return wafAuthenticatedRequest<T>(builtUrl, { type: 'json', ...wafOpts, headers, timeout, method });
  };
